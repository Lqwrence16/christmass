...
const GestureController = ({ onGesture, onMove, onStatus, openCam }: any) => {
  ...
  return (
    <>
      <video ref={videoRef} style={{ opacity: openCam ? 0.6 : 0, position: 'fixed', top: 0, right: 0, width: openCam ? 'min(200px, 40vw)' : '1px', zIndex: openCam ? 100 : -1, pointerEvents: 'none', transform: 'scaleX(-1)' }} playsInline muted autoPlay />
      <canvas ref={canvasRef} style={{ position: 'fixed', top: 0, right: 0, width: openCam ? 'min(200px, 40vw)' : '1px', height: openCam ? 'auto' : '1px', zIndex: openCam ? 101 : -1, pointerEvents: 'none', transform: 'scaleX(-1)' }} />
    </>
  );
};
